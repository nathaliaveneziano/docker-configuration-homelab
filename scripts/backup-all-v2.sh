...
# Localização: docker-configuration/scripts/backup-all-v2.sh
#
# Versão      : v2 — agendamento flexível via .env (BACKUP_SCHEDULE)
# Uso manual  : bash scripts/backup-all-v2.sh
...
# Nome do log inclui horário — suporta múltiplas execuções no mesmo dia
# sem sobrescrever o log anterior (ex: 02:00 e 14:00 geram arquivos distintos)
LOG_FILE="$LOG_DIR/backup-all_$(date +%Y%m%d_%H%M%S).log"
...
# Com múltiplas execuções por dia, 30 arquivos cobrem
